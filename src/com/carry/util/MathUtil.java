...
public class MathUtil {
	...
	public static int bigDecimalToInt(BigDecimal bd) {
		...
		try {
			val = Integer.parseInt(bd.toString());
		} catch (NumberFormatException nfe) {
			return 0;
		}
		return val;
	...
	public static double sumMoreDouble(double... value) {
		double sum = 0;
		if(value!=null && value.length>=2){
			for (int i = 0; i < value.length; i++) {
				sum = MathUtil.sumDouble(sum, value[i]);
			}
		}
		return sum;
	}
	...
	public static String doubleFormat(double d){
		...
		String doubleStr = "";
		doubleStr = df.format(d).equals("0")?"0":df.format(d);
